...
if  [ ! -n "$UMI_ENV" ] ;then
    output="test-zip-dist"
else
    output="${UMI_ENV}-test-zip-dist"
fi

# 删除之前打包好的项目

rm -rf ./*-zip-dist ./dist ./test*.zip
mkdir ./${output}
mkdir ./${output}/bin
...
if test -n "$UMI_ENV"; then
  if [ "$(uname)" == "Darwin" ];then
    sed -i '' 's/test/'${UMI_ENV}'_test/g' ./${output}/package.json
  else 
    sed -i 's/test/'${UMI_ENV}'_test/g' ./${output}/package.json
  fi
fi
...
if test -n "$UMI_ENV"; then
  if [ "$(uname)" == "Darwin" ];then
    sed -i '' 's/test/'${UMI_ENV}'_test/g' ./${output}/bin/stop.sh
  else 
    sed -i 's/test/'${UMI_ENV}'_test/g' ./${output}/bin/stop.sh
  fi
fi
